...
class RoomManager {
    ...
    async startScreenShare() {
        ...
        try {
            ...
            this.peers.forEach(({ pc }) => {
                ...
                // replace audio track with screen audio if available
                if (audioTracks.length > 0) {
                    const audioSender = pc.getSenders().find(s => s.track && s.track.kind === 'audio');
                    if (audioSender) {
                        audioSender.replaceTrack(audioTracks[0]).catch(err => console.warn('replaceAudioTrack failed', err));
                    }
                }
            });
            ...
            this.screenAudioTracks = audioTracks;
    ...
    async stopScreenShare() {
        ...
        if (cameraTrack || localAudioTrack) {
            this.peers.forEach(({ pc }) => {
                // restore video track
                if (cameraTrack) {
                    const videoSender = pc.getSenders().find(s => s.track && s.track.kind === 'video');
                ...
                if (localAudioTrack) {
                    const audioSender = pc.getSenders().find(s => s.track && s.track.kind === 'audio');
                    if (audioSender) audioSender.replaceTrack(localAudioTrack).catch(err => console.warn('replaceAudioTrack restore failed', err));
                }
            });
            if (cameraTrack) {
                this.ui.elements.localVideo.srcObject = this.localStream;
