...
class RoomManager {
    ...
    async startScreenShare() {
        ...
        try {
            ...
            // If both screen audio and mic available, create an AudioContext to mix them
            ...
            if (screenAudioTrack && localAudioTrack) {
                try {
                    ...
                    // Create sources from individual tracks
                    ...
                    // connect both to destination (simple mixing)
                    screenSource.connect(dest);
                    micSource.connect(dest);

                    mixedAudioTrack = dest.stream.getAudioTracks()[0];

                    // store mixer to clean up on stop
                    this._audioMixer = {
                        audioContext,
                        dest,
                        sources: [screenSource, micSource],
                        mixedTrack: mixedAudioTrack
                    };
                } catch (e) {
                    console.warn('Audio mixing failed, falling back to screen or mic only', e);
                    mixedAudioTrack = screenAudioTrack || localAudioTrack;
                }
            } else {
                mixedAudioTrack = screenAudioTrack || localAudioTrack || null;
            }

            // create a new display stream to show locally (screen video + mixed audio if any)
            ...
            // and replace/add audio sender with mixedAudioTrack when available
            this._screenAddedAudioSenders = this._screenAddedAudioSenders || new Map();
            this.peers.forEach((peer, userId) => {
                const pc = peer.pc;
                const videoSender = pc.getSenders().find(s => s.track && s.track.kind === 'video');
                if (videoSender) videoSender.replaceTrack(screenTrack).catch(err => console.warn('replaceTrack failed', err));

                if (mixedAudioTrack) {
                    let audioSender = pc.getSenders().find(s => s.track && s.track.kind === 'audio');
                    if (audioSender) {
                        audioSender.replaceTrack(mixedAudioTrack).catch(err => console.warn('replaceAudioTrack failed', err));
                    } else {
                        try {
                            const tmpStream = new MediaStream([mixedAudioTrack]);
                            const newSender = pc.addTrack(mixedAudioTrack, tmpStream);
                            const mapKey = userId || (peer && peer.id) || Math.random().toString(36).slice(2);
                            this._screenAddedAudioSenders.set(mapKey, newSender);
                        } catch (err) {
                            console.warn('addTrack for mixed audio failed', err);
                        }
                    }
                }
            });
            ...
            this.screenAudioTracks = { screen: screenAudioTrack, local: localAudioTrack, mixed: mixedAudioTrack };
    ...
    async stopScreenShare() {
        ...
        // restore local camera video to peers and restore/cleanup audio senders
        ...
        this.peers.forEach((peer, userId) => {
            ...
            if (cameraTrack) {
                const videoSender = pc.getSenders().find(s => s.track && s.track.kind === 'video');
                if (videoSender) videoSender.replaceTrack(cameraTrack).catch(err => console.warn('replaceTrack restore failed', err));
            }

            // restore or remove audio sender
            const audioSender = pc.getSenders().find(s => s.track && s.track.kind === 'audio');
            const mapKey = userId || (peer && peer.id);
            const addedSender = this._screenAddedAudioSenders ? this._screenAddedAudioSenders.get(mapKey) : null;

            if (localAudioTrack) {
                if (audioSender) {
                    // if an audio sender exists (may be the replaced one), replace with local mic
                    audioSender.replaceTrack(localAudioTrack).catch(err => console.warn('replaceAudioTrack restore failed', err));
                } else if (addedSender) {
                    // if we previously added a sender for this peer, replace its track with local audio
                    addedSender.replaceTrack ? addedSender.replaceTrack(localAudioTrack).catch(err => console.warn('replace on addedSender failed', err)) : null;
                }
            } else {
                // no local audio: if we added a sender, remove it
                if (addedSender) {
                    try {
            ...
            // cleanup stored sender entry
            if (this._screenAddedAudioSenders) this._screenAddedAudioSenders.delete(mapKey);
        });

        // cleanup audio mixer if present (stop mixed track and close AudioContext)
        if (this._audioMixer) {
            try {
                if (this._audioMixer.mixedTrack && typeof this._audioMixer.mixedTrack.stop === 'function') {
                    try { this._audioMixer.mixedTrack.stop(); } catch (e) { }
                }
                if (this._audioMixer.audioContext && typeof this._audioMixer.audioContext.close === 'function') {
                    this._audioMixer.audioContext.close().catch(() => { });
                }
            } catch (e) { /* ignore */ }
